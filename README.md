## 캡스톤 디자인 프로젝트 
  ...
  공식 프로젝트 명 : 비전서, 거리센서, 터치센서를 이용한 시각장애인 신호등 자동 감지 스마트 흰지팡이    
  팀명 :  알아서 잘하자    
  제품명 : 같이봐
...
## Demo Day Preparation : What will we demonstrate?
  ### 비전 센서, 진동 모터 모듈
  + 컴퓨터 모니터를 최대한 신호등과 비슷한 방식으로 세워둔다.
  + 세워둔 모니터에 보행자 신호등과 최대한 비슷한 인터페이스를 출력하여 비전센서로 신호등의 색을 검출한다.
  + 검출된 정보를 진동을 통해 손으로 전달한다. 빨간불과 파란불 사이의 진동 차이를 둬서 구분 할 수 있게 한다.
  + 빨간불 (깜박이는 진동), 파란불 (지속되는 진동)
  ### 초음파 거리 감지 센서, 버저
  + 벽 밎 단순 지팡이로는 감지에 실패할 수 있는 장애물들을 설치한다. (최대한 현장감이 있는 장애물을 준비한다.)
  + 초음파 센서로 장애물들과 다가오는 보행자를 검출하여 1m 이내에 다가오면 버저에서 소리를 출력하여 경고를 전달한다.
  ### 터치 센서, 음성 녹음 및 플레이 모듈, GPS 모듈
  + 도움이 필요한 경우 터치 센서를 누르는 상황을 연출한다. 다친 상황을 연출하여 터치 센서를 통해 도움을 요청한다.
  + 터치 센서를 누르는 경우, 녹음된 소리를 플레이 모듈에서 출력한다.
  + 터치 센서를 누르는 경우, GPS 모듈을 통해 위치정보를 서버에 전송한다. 서버에 전송된 자료가 정확한 자료인지, 위도 경도로 지도에 검색해서 현재 위치와 비교하는 모습을 보여준다.
  + 서버에 단시간에 여러번의 도움 요청을 한다면, 도움을 줄 수 있다는 것을 보여준다. (추후에 ALert 등을 뜨게 하여 도움이 필요함을 보여줄 수 있다.)
